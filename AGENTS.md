...
Docs-site scaffolding (MkDocs) exists, plus `.github/workflows/ci.yml` which builds/verifies every stage. Stages built so far, each a complete, buildable Maven project under `stages/NN-Stage-Name/`:

- **01 - Getting Started** (plain): adds the `mango4j-crypto` dependency.
- **02 - Encrypt a Field** (starter/complete): encrypts a single `cardNumber` field.
- **03 - Real Encryption** (starter/complete): swaps the Base64 test delegate for the real `PBKDF2EncryptionService`, configuring the mechanism via the `CryptoKey`'s `type`/`configuration`.
- **04 - Key Aliases and Configs** (starter/complete): the provider now knows two keys; `getCurrentEncryptionKey()`/`getById()` resolve "current" by alias instead of returning the same hardcoded key regardless of the id asked for.
- **05 - AWS KMS Delegate** (starter/complete): swaps `PBKDF2EncryptionService` for the real `AwsKmsEncryptionServiceDelegate` (`mango4j-crypto-aws-kms-delegate`), backed by a hand-written `FakeKmsClient` (implements the AWS SDK's `KmsClient` interface directly, real AES/GCM underneath, no network/credentials/Docker) - same pattern as every earlier fake delegate in the workshop. Runs identically in CI and locally. Also includes `RealAwsKmsClientExample`, a real `KmsClient.builder().build()` construction that compiles (proving the wiring) but is deliberately never called from `Main` - CI verifies it builds without ever needing real AWS credentials.
- **06 - Introducing HMACs** (starter/complete): `@Hmac`/`@HmacKeyId` added alongside `@Encrypt`, a dedicated HMAC key resolved via `getCurrentHmacKeys()`, proving the HMAC is deterministic across runs while the ciphertext isn't - stops short of using it for search/uniqueness.
- **07 - Single HMAC Strategy** (starter/complete): a `PaymentCardStore` searches/enforces-uniqueness by HMAC equality, then reproduces live the strategy's structural failure - a duplicate encrypted under a rotated HMAC key is silently accepted, since the store only ever compares against one HMAC per record.
- **08 - List HMAC Strategy** (starter/complete): `PaymentCardEntity` implements `Lookup`/`Unique` instead of single HMAC fields, `getCurrentHmacKeys()` can return more than one active key at once, and the store's comparison walks both lists looking for any shared value - proven by rotating the HMAC key with the old one kept active, correctly catching the exact duplicate that slipped through in Stage 7.
- **09 - Key Rotation** (starter/complete): a single, live `InMemoryCryptoKeyProvider` instance whose `rotateEncryptionKeyTo()` repoints "current" mid-run - contrasted with Stage 4's two statically-configured providers - proving new writes pick up the new key while data encrypted before the rotation still decrypts under the old one; phase 1 only, migrating existing data is deferred to Rekeying: Encryption.
- **10 - Rekeying: Encryption** (starter/complete): uses mango4j-crypto's real production `RekeyScheduler`/`RekeyService`/`RekeyCryptoKeyManager` (not a hand-rolled sweep) via the `KEY_OFF` path - marking an old key for retirement drives an async background job that sweeps every record onto the current key and calls back to delete the old key once done. Requires two `CountDownLatch`es (no synchronous return to block on) and an explicit `System.exit()` (the scheduler's background thread never stops on its own). Note: the mirror-image `KEY_ON` path has a confirmed bug in mango4j-crypto - see [mango4j-crypto#44](https://github.com/bitstep-ie/mango4j-crypto/issues/44) - so this stage deliberately uses `KEY_OFF` instead.
- **11 - Rekeying: HMACs** (starter/complete): uses the real `RekeyScheduler` (not a hand-rolled sweep) via `RekeyListHmacFieldStrategy`, two phases driven by `CryptoKey.rekeyMode`: mark the new HMAC key `KEY_ON` (sweep everyone onto it, additive), then clear it and mark the old key `KEY_OFF` (retire it) - verified empirically that this HMAC path, unlike encryption's `KEY_ON` (see Stage 10), is not affected by the mango4j-crypto#44 bug. Also notes that key retirement doesn't purge the old key's now-stale HMAC entries from existing records (`RekeyEvent.Type.PURGE_REDUNDANT_HMACS_ASSOCIATED_WITH_KEY` is a separate, uncovered step).
- **12 - Migrating Unencrypted Fields** (starter/complete): two entities with a non-transient `@Encrypt` field, fixed via `@EnableMigrationSupport` - demonstrates the framework's real behavior here (a dated, logged exception to the "must be transient" rule, not a dual-representation reader). Also covers the rest of the lifecycle: a backfill sweep that encrypts every already-existing legacy record, then a cutover to `MigratedEntity` (plain `@Encrypt private transient`, no migration annotation) once every record's been backfilled - the code-level signal that the old plaintext column is safe to drop. Contrasted with `talk/naive-migration/`'s hand-rolled backfill demo. Last stage in the hands-on arc.

All 12 stages now have real, working, CI-verified content — no placeholders remain. `talk/` holds supporting code samples for the conceptual talk that precedes the hands-on workshop (see [Talk supporting samples](#talk-supporting-samples) below). The `.gitignore` targets a Java/Maven project (`*.class`, `*.jar`, `*.war`, `target/`, etc.), so workshop code is implemented in Java/Maven.
