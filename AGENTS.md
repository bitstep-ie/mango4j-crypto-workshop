...
- **12 - Migrating Unencrypted Fields** (starter/complete): two entities with a non-transient `@Encrypt` field, fixed via `@EnableMigrationSupport` - demonstrates the framework's real behavior here (a dated, logged exception to the "must be transient" rule, not a dual-representation reader), contrasted with `talk/naive-migration/`'s hand-rolled backfill demo for the rest of that story. Last stage in the hands-on arc.
