...
- **11 - Rekeying: HMACs** (starter/complete): a mutable active-HMAC-key list (`addActiveHmacKey()`/`removeActiveHmacKey()`), a sweep that refreshes each record's HMAC list by calling `encrypt()` again, and a live reproduction of the teardown-ordering bug - retiring the old key before the sweep reaches every record makes the un-swept record(s) silently unfindable.
