...
- **10 - Rekeying: Encryption** (starter/complete): a `RekeySweep` decrypts/re-encrypts every already-persisted record still on an old key, with an `isAlreadyOnKey()` check that makes re-running the sweep after it's finished a safe no-op instead of redundant work.
