...
Docs-site scaffolding (MkDocs), `stages/01-Getting-Started/` (a plain stage: adds the `mango4j-crypto` dependency), `stages/02-Encrypt-a-Field/` (a starter/complete stage: encrypts a single `cardNumber` field), `stages/03-Real-Encryption/` (a starter/complete stage: swaps the Base64 test delegate for the real `PBKDF2EncryptionService`, configuring the mechanism via the `CryptoKey`'s `type`/`configuration`), `stages/04-Key-Aliases-and-Configs/` (a starter/complete stage: the provider now knows two keys, and `getCurrentEncryptionKey()`/`getById()` resolve "current" by alias instead of returning the same hardcoded key regardless of the id asked for), `stages/06-Introducing-HMACs/` (a starter/complete stage: `@Hmac`/`@HmacKeyId` added alongside `@Encrypt`, a dedicated HMAC key resolved via `getCurrentHmacKeys()`, proving the HMAC is deterministic across runs while the ciphertext isn't - stops short of using it for search/uniqueness), `stages/07-Single-HMAC-Strategy/` (a starter/complete stage: a `PaymentCardStore` searches/enforces-uniqueness by HMAC equality, then reproduces live the strategy's structural failure - a duplicate encrypted under a rotated HMAC key is silently accepted, since the store only ever compares against one HMAC per record), `stages/08-List-HMAC-Strategy/` (a starter/complete stage: `PaymentCardEntity` implements `Lookup`/`Unique` instead of single HMAC fields, `getCurrentHmacKeys()` can return more than one active key at once, and the store's comparison walks both lists looking for any shared value - proven by rotating the HMAC key with the old one kept active, correctly catching the exact duplicate that slipped through in Stage 7), and `stages/09-Key-Rotation/` (a starter/complete stage: a single, live `InMemoryCryptoKeyProvider` instance whose `rotateEncryptionKeyTo()` repoints "current" mid-run - contrasted with Stage 4's two statically-configured providers - proving new writes pick up the new key while data encrypted before the rotation still decrypts under the old one; phase 1 only, migrating existing data is deferred to Rekeying: Encryption) exist, plus `.github/workflows/ci.yml` which builds/verifies every stage. Stage 5 (`docs/stages/05-aws-kms-delegate.md`) and stages 10-12 (`docs/stages/10-rekeying-encryption.md` through `docs/stages/12-migrating-unencrypted-fields.md`) exist only as docs-only placeholder pages, each with an Overview admonition and a "Content coming soon" outline of what the stage should teach, mapped from the talk's chapter arc plus migration — no `stages/NN-.../` Maven folders exist for them yet; write those when a stage's content is actually drafted. Stage 5 is additionally blocked on a decision about local/CI testing against AWS KMS (e.g. LocalStack) before real code can be written. `talk/` holds supporting code samples for the conceptual talk that precedes the hands-on workshop (see [Talk supporting samples](#talk-supporting-samples) below). The `.gitignore` targets a Java/Maven project (`*.class`, `*.jar`, `*.war`, `target/`, etc.), so workshop code is implemented in Java/Maven.
