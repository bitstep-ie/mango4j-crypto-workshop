...
Docs-site scaffolding (MkDocs), `stages/01-Getting-Started/` (a plain stage: adds the `mango4j-crypto` dependency), `stages/02-Encrypt-a-Field/` (a starter/complete stage: encrypts a single `cardNumber` field), `stages/03-Real-Encryption/` (a starter/complete stage: swaps the Base64 test delegate for the real `PBKDF2EncryptionService`, configuring the mechanism via the `CryptoKey`'s `type`/`configuration`), and `stages/04-Key-Aliases-and-Configs/` (a starter/complete stage: the provider now knows two keys, and `getCurrentEncryptionKey()`/`getById()` resolve "current" by alias instead of returning the same hardcoded key regardless of the id asked for) exist, plus `.github/workflows/ci.yml` which builds/verifies every stage. Stages 5-12 (`docs/stages/05-aws-kms-delegate.md` through `docs/stages/12-migrating-unencrypted-fields.md`) exist only as docs-only placeholder pages, each with an Overview admonition and a "Content coming soon" outline of what the stage should teach, mapped from the talk's chapter arc plus migration — no `stages/NN-.../` Maven folders exist for them yet; write those when a stage's content is actually drafted. `talk/` holds supporting code samples for the conceptual talk that precedes the hands-on workshop (see [Talk supporting samples](#talk-supporting-samples) below). The `.gitignore` targets a Java/Maven project (`*.class`, `*.jar`, `*.war`, `target/`, etc.), so workshop code is implemented in Java/Maven.
