...
public class InMemoryCryptoKeyProvider implements CryptoKeyProvider {
    ...
    public List<CryptoKey> getCurrentHmacKeys() {
        ...
        // TODO:START current-hmac-keys
        /* TODO: Return the current HMAC key, resolved the same way as the
         * encryption key: by id, through getById(). This returns a List
         * because a later stage (List HMAC Strategy) supports more than one
         * active HMAC key at once - for now, a single-element list.
         */
        currentHmacKeys = List.of(getById(CURRENT_HMAC_KEY_ID));
        // TODO:END current-hmac-keys
