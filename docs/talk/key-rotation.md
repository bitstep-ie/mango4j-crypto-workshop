...
Changing an encryption key is, by itself, straightforward: add the new key to your system and start using it for all writes going forward. Concretely, that means changing what "the current encryption key" resolves to (see [Key Aliases & Key Configs](key-aliases.md)) — nothing else in your application code changes.
...
**A data retention window doesn't remove the need for this.** It's tempting to reason that if the key period is 10 years but data retention is only 5, rekey support is unnecessary — the key could just be swapped every 10 years. Consider what happens in year 11: the key changes, and 5 years of records (years 6–10) are still sitting on the old key. Search doesn't actually break here — those records stay findable as long as every search hashes the term with every key still known, old and new (see [Introducing HMACs](introducing-hmacs.md)) — but that only works by retaining, and searching against, a list of old keys that never stops growing. Unique constraints fare worse: under the [Single HMAC Strategy](single-hmac.md), a duplicate can slip in under any of those years' keys at any point, and no amount of searching with old keys closes that gap — only rekeying does. Rekey support is needed regardless of how long data is retained, not only when retention exceeds the key period.
