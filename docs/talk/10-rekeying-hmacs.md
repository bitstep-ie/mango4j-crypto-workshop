...
Chapter 9 covered re-encrypting a record's ciphertext from an old key to a new one. HMACs need the same eventual outcome — get everything off old keys — but the process looks different, because a HMAC isn't just replaced in place. Under the List HMAC Strategy (Chapter 8), a record can legitimately have HMACs from *multiple* keys stored simultaneously, and the rekey job has to *add* new entries without disturbing old ones that other in-flight operations might still depend on.

This additive requirement is exactly why HMAC rekeying, in practice, tends to only be well-supported for entities using the List HMAC Strategy. Encryption-only rekeying (Chapter 9) doesn't care which HMAC strategy an entity uses — it works regardless; HMAC rekeying specifically needs the list shape to do this safely.

## The process

1. **Add the new HMAC key** to the active key list — this is rotation phase 1 from Chapter 5, nothing new here.
2. **Wait for the key cache expiry** to pass, so every application instance is actually using the new key for writes (the same caching hazard Chapter 7 described for the multi-instance search race).
...
The rest of this session moves from theory to hands-on practice, working through the numbered stages in this workshop's repository — starting from a plain project with a crypto framework added as a dependency, and building up field encryption, HMAC search, and (as stages are added) key rotation and rekeying, one exercise at a time. Each stage maps back to a concept from this talk: expect to see field-level encryption and structured ciphertext from Chapters 3–4 first, HMAC strategies from Chapters 6–8 next, and rotation/rekeying from Chapters 5, 9–10 as the workshop progresses.
