...
5. **Wait for the sweep to finish**, then remove the old key from the active key list.
    ...
    "Wait for the sweep to finish" is a real, ongoing cost, not just a sequencing rule to get right once. It means keeping the old key valid, loaded, and known to *every* application instance ([the same caching hazard step 2 and Single HMAC Strategy described](single-hmac.md), now applying to *keeping* a key rather than *picking up* one) for as long as the sweep takes to reach every record on it. For a large database, that sweep is a substantial, long-running operation in its own right, potentially the biggest task in the whole rotation, not a quick background chore.
