...
5. **Wait for the sweep to finish**, then remove the old key from the active key list.
    ...
    "Wait for the sweep to finish" is a real, ongoing cost, not just a sequencing rule to get right once. It means keeping the old key valid, loaded, and known to *every* application instance for as long as the sweep takes to reach every record on it, and for a large database, that sweep is a substantial, long-running operation in its own right, potentially the biggest task in the whole rotation, not a quick background chore. It also depends on an assumption that's easy to lose track of: every instance actually has the old key loaded throughout that entire window. An instance that restarts partway through and reloads its key configuration from a source that's already had the old key removed, or one that was never told about it in the first place, reintroduces the exact same silent-unfindable failure this section is about, just triggered by a deployment or config gap instead of an early `removeActiveKey` call. The old key isn't safe to drop from anywhere until the sweep and every instance's key knowledge agree it's done, and for a large enough database that can be the long pole in the whole rekey.
