...
- **A concurrency window inside `save()`.** Between the encrypt step and the decrypt step, the *same* entity instance, not a copy, sits there with ciphertext in a field the rest of the application expects to hold plaintext. In a single-threaded flow that window is invisible and harmless. In a multi-threaded application where that entity can be read concurrently, any read that lands inside the window silently sees ciphertext where it expects plaintext, no lock, no exception, no signal that the object is mid-mutation; it just quietly hands out the wrong representation to whoever happens to read it at the wrong moment. In a typical Spring/JPA app this window rarely gets a chance to matter: each request thread usually works with its own entity instance, so two threads racing on the *same* object isn't the normal shape of the code. But `save()` and `load()` here, like `encrypt()`/`decrypt()` themselves, are POJO operations with no idea whether the object they're handed is request-scoped or not, so the moment an entity instance is shared outside that pattern, a cache, a background job, anything holding a reference across threads, the window is real again with nothing in the ALE layer protecting against it.
