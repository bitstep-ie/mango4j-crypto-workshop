...
Quick answers to questions that come up repeatedly when discussing ALE, split into general concepts and questions specific to mango4j-crypto, the framework this workshop uses. Each one links to the chapter (or the workshop) that covers the full reasoning.
...
### HMAC is deterministic by definition — does this framework change that?

No. Determinism (same input, same key, always the same output) is a property of the HMAC algorithm itself — HMAC-SHA256 or whichever construction is in use — not something a particular framework adds or removes. A given (value, key) pair always produces the same HMAC, full stop. What mango4j-crypto's List HMAC Strategy varies isn't whether a HMAC is deterministic — it's *how many* HMACs get computed and stored for a value, and under how many different keys. It stores one HMAC per active key rather than one HMAC overall, but each of those individual HMACs is still exactly as deterministic as it always was — you're just keeping several deterministic answers to "what's the HMAC of this value under key K", one per key, side by side.

### What HMAC strategies does mango4j-crypto support?

Three, chosen per entity via a class-level annotation: Single HMAC, Double HMAC, and List HMAC. mango4j-crypto's own authors recommend List HMAC as the default unless there's a strong reason not to use it — see [Single HMAC Strategy](single-hmac.md) and [List HMAC Strategy](list-hmac.md) for the trade-offs.
